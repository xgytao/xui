...
PATH_FOR_GEO_IP='/usr/local/xui/bin/geoip.dat'
PATH_FOR_CONFIG='/usr/local/xui/bin/config.json'
PATH_FOR_GEO_SITE='/usr/local/xui/bin/geosite.dat'
...
update_shell() {
    wget -O /usr/bin/xui -N --no-check-certificate https://github.com/xgytao/xui/raw/master/xui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
#add for cron jobs,including sync geo data,check logs and restart xui
cron_jobs() {
    clear
    echo -e "
  ${green}定时任务管理${plain}
...
enable_auto_update_geo() {
    ...
    echo "00 4 */2 * * xui geo > /dev/null" >>/tmp/crontabTask.tmp
...
disable_auto_update_geo() {
    crontab -l | grep -v "xui geo" | crontab -
    if [[ $? -ne 0 ]]; then
        LOGI "取消xui 自动更新geo数据失败"
    else
        LOGI "取消xui 自动更新geo数据成功"
    fi
}
...
enable_auto_clear_log() {
    ...
    if [[ -f ${accessfilePath} ]]; then
        crontab -l >/tmp/crontabTask.tmp
        echo "30 4 */2 * * xui clear ${accessfilePath} > /dev/null" >>/tmp/crontabTask.tmp
        crontab /tmp/crontabTask.tmp
        rm /tmp/crontabTask.tmp
    ...
    if [[ -f ${errorfilePath} ]]; then
        crontab -l >/tmp/crontabTask.tmp
        echo "30 4 */2 * * xui clear ${errorfilePath} > /dev/null" >>/tmp/crontabTask.tmp
        crontab /tmp/crontabTask.tmp
        rm /tmp/crontabTask.tmp
...
disable_auto_clear_log() {
    crontab -l | grep -v "xui clear" | crontab -
    if [[ $? -ne 0 ]]; then
...
show_usage() {
    ...
    echo "xui clear        - 清除 xui 日志"
    echo "xui geo          - 更新 xui geo数据"
    echo "xui cron         - 配置 xui 定时任务"
    echo "------------------------------------------"
}
